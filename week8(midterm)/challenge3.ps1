...
# this filters only the logs that contain anything from indicators
$susLogs = $logs | Where-Object {
    $found = $false
    foreach ($indicator in $Indicators) {
        if ($_.Page -like "*$indicator*") {
            $found = $true
            break
        }
    }
